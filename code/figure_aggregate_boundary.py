...
def paper_xn(A,C,G,L,B1,B2):
    ...
    return ((d11*(d4-d22)+d21*d13)*(A-C)/den,
            (d21*(d4-d12)+d11*d23)*(A-C)/den)
...
def paper_xc(A,C,G,L,B1,B2):
    ...
    return ((p11*(p4-p22)+p21*p3)*(A-C)/den,
            (p21*(p4-p12)+p11*p3)*(A-C)/den)


def da(B1,B2,G):
    xn=paper_xn(100.0,50.0,G,0.0,B1,B2)
    xc=paper_xc(100.0,50.0,G,0.0,B1,B2)
    return xn[0]+xn[1]-xc[0]-xc[1]
...
def main():
    ...
    ax.annotate(r'$(1/2,1/2)$',xy=(0.5,0.5),xytext=(0.56,0.43),fontsize=9,
